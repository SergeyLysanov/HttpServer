...
string getDecodedPath(string sRequest)
{
    ...
    //char* output = (char*)malloc(strlen(sPath.c_str())+1);
    //urldecode2(output, sPath.c_str());
...
void writeHeader(HttpHeader *pHttpHeader, evbuffer *pOutput)
{
    ...
    if(pHttpHeader->eResponseType == eOK)
    {
        ...
        sResponse.append("Server: MyServer\n");

        //Parse content type
        string sContent;
        if(pHttpHeader->eContentType == eHTML){
            sContent = "text/html";
        }
        else if(pHttpHeader->eContentType == eJPG){
            sContent = "image/jpeg";
        }
